...
export class TerrenosNewComponent implements OnInit {
  ...
  activo= this.fb.group({
    id_activo:0,
    id_cuenta: [171,Validators.required],
  ...
  getErrorMessages(field: string){
    let message;
    if(this.terrenos.get(field).errors.required){
      message = 'Requiere datos...';
    }
    return message;
  }
  isValidFields(field: string): boolean {
    return ((this.terrenos.get(field).touched || this.terrenos.get(field).dirty) && 
    !this.terrenos.get(field).valid);
  }
    ...
    if (this.actirouter.snapshot.paramMap.get('id')){
      ...
      .subscribe(
        res=>{console.log('res get id', res)
              ...
              console.log('res acti', this.activo)
              //this.activo=res
              this.adquisicion.setValue(res.adquisicion_activo)
              console.log('res adqui', this.adquisicion)
              //this.adquisicion=res.adquisicion_activo
              this.terrenos.setValue(res.terrenos)
    ...
    .subscribe(
      ...
      if(this.adquisicion.status == "VALID"){
        ...
        .subscribe(
          ...
          if(this.terrenos.status == "VALID"){
          this.activoservice.putTerrenos(this.terrenos.value)
          .subscribe(
            res=>{console.log('resput adquiacti', res)
