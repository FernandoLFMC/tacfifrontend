...
export class EdificiosNewComponent implements OnInit {
  ...
  activo= this.fb.group({
    id_activo:"",
    id_cuenta: [172,Validators.required],
  ...
  edificios= this.fb.group ({
    id_activo: 0,
    cod_catastro:["",Validators.required],
    superficie: ["",Validators.required],
    matricula_ddrr:["",Validators.required],
  ...
  getErrorMessages(field: string){
    let message;
    if(this.edificios.get(field).errors.required){
      message = 'Requiere datos...';
    }
    return message;
  }
  isValidFields(field: string): boolean {
    return ((this.edificios.get(field).touched || this.edificios.get(field).dirty) && 
    !this.edificios.get(field).valid);
  }
    ...
    if (this.actirouter.snapshot.paramMap.get('id')){
      ...
      .subscribe(
        res=>{console.log('res get id', res)
              ...
              this.edificios.setValue(res.edificios)
    ...
    if(this.activo.status == "VALID"){
      ...
      .subscribe(
        res=>{
          ...
          this.edificios.patchValue({id_activo : res.id_activo})
          ...
          if(this.adquisicion.status == "VALID"){
            ...
            .subscribe(
              res=>{console.log('res AdquiActi', res)
                if(this.edificios.status == "VALID"){
                  this.activoservice.createedificios(this.edificios.value)
                  .subscribe(
    ...
    .subscribe(
      ...
      if(this.adquisicion.status == "VALID"){
        ...
        .subscribe(
          ...
          if(this.edificios.status == "VALID"){
          this.activoservice.putEdificios(this.edificios.value)
          .subscribe(
            res=>{console.log('resput adquiacti', res)
