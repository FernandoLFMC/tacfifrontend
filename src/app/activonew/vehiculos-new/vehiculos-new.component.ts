...
export class VehiculosNewComponent implements OnInit {
  ...
  constructor(private activoservice:ActivoService,
    ...
    activo= this.fb.group({
      id_activo:"",
      id_cuenta: [176,Validators.required],
    ...
    vehiculos= this.fb.group({
      id_activo: 0,
      ruat:["",Validators.required],
      nro_placa:["",Validators.required],
      marca:["",Validators.required],
      tipo:["",Validators.required],
      color:["",Validators.required],
      modelo:["",Validators.required],
      nro_chasis:["",Validators.required],
      nro_motor:"",
      procedencia:"",
    })
    ...
    getErrorMessages(field: string){
      let message;
      if(this.vehiculos.get(field).errors.required){
        message = 'Requiere datos...';
      }
      return message;
    }
    isValidFields(field: string): boolean {
      return ((this.vehiculos.get(field).touched || this.vehiculos.get(field).dirty) && 
      !this.vehiculos.get(field).valid);
    }
      ...
      if (this.actirouter.snapshot.paramMap.get('id')){
        ...
        .subscribe(
          res=>{console.log('res get id', res)
                ...
                this.vehiculos.setValue(res.vehiculos)
      ...
      if(this.activo.status == "VALID"){
        ...
        .subscribe(
          res=>{
            ...
            this.vehiculos.patchValue({id_activo : res.id_activo})
            if(this.adquisicion.status == "VALID"){
              ...
              .subscribe(
                res=>{console.log('res AdquiActi', res)
                  if(this.vehiculos.status == "VALID"){
                    this.activoservice.createvehiculos(this.vehiculos.value)
                    .subscribe(
      ...
      .subscribe(
        ...
        if(this.adquisicion.status == "VALID"){
          ...
          .subscribe(
            ...
            if(this.vehiculos.status == "VALID"){
            this.activoservice.putVehiculos(this.vehiculos.value)
            .subscribe(
              res=>{console.log('resput adquiacti', res)
